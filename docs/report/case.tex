...
The graphical user interface of the application is quite simplistic, this as two primary reason. The first being that this prototype is more focused on the technology and viability of a bio-feedback system based on a Wii Remote and an Android device, then the user interface. The application is intended those who do not have great coordination or muscle control, so we are trying to keep the interaction with the phone to a minimum, and when they do need to interact with it, then it should be easy and very simple.
...
Once a successful connection has been established the Wii Remote should be calibrated in order to achieve optimal accuracy. Through the use of the gyro data the Madgwick algorithm is able to correct inaccurate data, but the accuracy is not satisfactory for a bio-feedback system unless the WiiMote is calibrated. % CONFIRM THIS
Calibration is achieved by putting the Wii Remote on a flat surface with the face up, and then pressing the "Calibrate" button. Calibration is instantaneous and the application is ready for use.

WRITE ABOUT THE USE OF THE APPLICATION
