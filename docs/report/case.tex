...
The algorithm returns the result as a Quaternion that need to be stored and converted to Euler angles. The formulas for converting the Quaternion to Euler angles are provided in the Madgwick paper, but the roll angle was incorrect and made no sense. After looking at Tryggestads implementation of euler angle conversion we found that Madgwick had use the wrong quaternion values for the roll value conversion.
