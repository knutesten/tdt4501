...
/** 
 * Java implementation of the MadgwickAHRS algorithm
...
public class MadgwickAHRS {
	...
	public void update(float gx, float gy, float gz, float ax, float ay, float az)
	{
        ...
        norm = 1f / FloatMath.sqrt(s1 * s1 + s2 * s2 + s3 * s3 + s4 * s4);    // normalise step magnitude
        s1 *= norm;
        s2 *= norm;
        s3 *= norm;
        s4 *= norm;
        ...
        norm = 1f / FloatMath.sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4);    // normalise quaternion
	...
	public void update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
    {
        ...
        _2bx = FloatMath.sqrt(hx * hx + hy * hy);
